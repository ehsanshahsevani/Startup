...
builder.Services.Scan(scan => scan
	.FromAssemblyOf<CheckCategoryIdActionFilter>()
	.AddClasses(classes => classes
		.Where(type =>
			typeof(InfrastructureSeedworks.ActionFilter.IBaseAsyncActionFilter).IsAssignableFrom(type) &&
...
using (var scope = app.Services.CreateScope())
{
	...
	List<ActionViewModel> actions =
		ControllerSeedworks.BaseApiController
			.ActionScanner.ScanActions(Assembly.GetExecutingAssembly(), Domain.Base.ServerKeyConstant.Key);

	ActionService actionService = new ActionService();
	
	await actionService.AddAsync(
		actions,
		serverId: Domain.Base.ServerKeyConstant.Key,
	...
	var initialData =
	    new Infrastructure.InitialData(builder.Configuration, unitOfWork);

	await initialData.CreateTagPageSettingsAsync();
	
	await initialData.CreateAccountCodingAsync();
	await initialData.CreateTalaSootFeeAsync();
	await initialData.CreateTypeRoleGoldAsync();
	await initialData.CreateTypeRoleMoneyAsync();
	await initialData.CreateGoldTreasuryAsync();
	await initialData.CreateTalaSootSettingsAsync();
	await initialData.CreateTalaSootBankAccountAsync();
