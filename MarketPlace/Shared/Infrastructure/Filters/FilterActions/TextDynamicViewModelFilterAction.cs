...
public class TextDynamicViewModelFilterAction :
	InfrastructureSeedworks.ActionFilter.IBaseAsyncActionFilter
{
	public IUnitOfWork UnitOfWork { get; }

	public TextDynamicViewModelFilterAction(IUnitOfWork unitOfWork)
	{
		...
		ActionExecutingContext context, ActionExecutionDelegate next)
	{
		...
		if (tryGetValue == true && modelObject is TextDynamicRequestViewModel model)
		{
			...
			if (result.IsSuccess == true)
			{
				...
				List<TagPageSetting> listTags =
					await UnitOfWork
						.TagPageSettingRepository
						.FindByStartWithNamesAsync(nameof(Resources.DataDictionary.TextDynamic));

				if (listTags.Any() == true)
				{
					context.HttpContext.Items[nameof(Resources.DataDictionary.TextDynamic)] = listTags;
				}
				else
				{
					var errorMessage =
						string.Format(Resources.Messages.NotFoundError, Resources.DataDictionary.TextDynamic);
