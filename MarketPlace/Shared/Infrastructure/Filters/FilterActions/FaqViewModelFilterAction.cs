...
public class FaqViewModelFilterAction :
	InfrastructureSeedworks.ActionFilter.IBaseAsyncActionFilter
{
	public IUnitOfWork UnitOfWork { get; }

	public FaqViewModelFilterAction(IUnitOfWork unitOfWork)
	{
		...
		ActionExecutingContext context, ActionExecutionDelegate next)
	{
		...
		if (tryGetValue == true && modelObject is FaqRequestViewModel model)
		{
				...
				else
				{
					TagPageSetting? faqTagPageSetting =
						await UnitOfWork
							...
							.FindByNameAsync(nameof(Resources.DataDictionary.Faq));

					if (faqTagPageSetting is not null)
					{
						context.HttpContext.Items[nameof(Resources.DataDictionary.Faq)] = faqTagPageSetting;
					}
					else
					{
						var errorMessage =
							string.Format(Resources.Messages.NotFoundError, Resources.DataDictionary.Faq);
