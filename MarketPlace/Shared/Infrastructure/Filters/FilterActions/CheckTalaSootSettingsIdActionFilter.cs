...
public class CheckTalaSootSettingsIdActionFilter : InfrastructureSeedworks.ActionFilter.IBaseAsyncActionFilter
{
	public IUnitOfWork UnitOfWork { get; }

	public CheckTalaSootSettingsIdActionFilter(IUnitOfWork unitOfWork)
	{
	...
	public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
	{
		...
		else
		{
			var entity =
				await UnitOfWork
					.RoleGoldRepository.FindAsync(id);

			if (entity is null)
			{
				var errorMessage = string.Format(
					Resources.Messages.NotFoundError, Resources.DataDictionary.RoleGold);
