...
public class RoleMoneyViewModelFilterAction :
    IBaseAsyncActionFilter
{
    public RoleMoneyViewModelFilterAction(IUnitOfWork unitOfWork)
    {
        ...
        ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        if (tryGetValue == true && modelObject is RoleMoneyRequestViewModel model)
        {
            ...
            if (result.IsSuccess == true)
            {
                ...
                //     await UnitOfWork.RoleMoneyRepository.IsOkForAddAsync(model);
                //
                ...
                {
                    var entity =
                        await UnitOfWork.RoleMoneyRepository.FindAsync(model.Id);

                    if (entity is null)
                    {
                        var errorMessage =
                            string.Format(Messages.NotFoundError, DataDictionary.RoleMoney);
