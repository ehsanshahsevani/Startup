...
public class CartItemViewModelFilterAction :
    IBaseAsyncActionFilter
{
    public CartItemViewModelFilterAction(IUnitOfWork unitOfWork)
    {
        ...
        ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        if (tryGetValue && modelObject is CartItemRequestViewModel model)
        {
            ...
            if (result.IsSuccess)
            {
                ...
                var checkForAddResult =
                    await UnitOfWork.CartItemRepository.IsOkForAddAsync(model);
                ...
                {
                    var entity =
                        await UnitOfWork.CartItemRepository.FindAsync(model.Id);

                    if (entity is null)
                    {
                        var errorMessage =
                            string.Format(Messages.NotFoundError, DataDictionary.CartItem);
