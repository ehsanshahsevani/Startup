...
public class BranchViewModelFilterAction :
    InfrastructureSeedworks.ActionFilter.IBaseAsyncActionFilter
{
    public IUnitOfWork UnitOfWork { get; }

    public BranchViewModelFilterAction(IUnitOfWork unitOfWork)
    {
        ...
        ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        if (tryGetValue == true && modelObject is BranchRequestViewModel model)
        {
            ...
            if (result.IsSuccess == true)
            {
                ...
                var checkForAddResult =
                    await UnitOfWork.BranchRepository.IsOkForAddAsync(model);
                ...
                {
                    var entity =
                        await UnitOfWork.BranchRepository.FindAsync(model.Id);

                    if (entity is null)
                    {
                        var errorMessage =
                            string.Format(Resources.Messages.NotFoundError, Resources.DataDictionary.Branch);
