...
public class CheckRoleMoneyIdActionFilter: InfrastructureSeedworks.ActionFilter.IBaseAsyncActionFilter
{
    public IUnitOfWork UnitOfWork { get; }

    public CheckRoleMoneyIdActionFilter(IUnitOfWork unitOfWork)
    {
    ...
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        else
        {
            var entity =
                await UnitOfWork
                    .RoleMoneyRepository.FindAsync(id);

            if (entity is null)
            {
                var errorMessage = string.Format(
                    Resources.Messages.NotFoundError, Resources.DataDictionary.RoleMoney);
