...
public class BannerViewModelFilterAction :
	InfrastructureSeedworks.ActionFilter.IBaseAsyncActionFilter
{
	public IUnitOfWork UnitOfWork { get; }

	public BannerViewModelFilterAction(IUnitOfWork unitOfWork)
	{
		...
		ActionExecutingContext context, ActionExecutionDelegate next)
	{
		...
		if (tryGetValue == true && modelObject is BannerRequestViewModel model)
		{
			...
			if (result.IsSuccess == true)
			{
				...
				List<TagPageSetting> listTags =
					await UnitOfWork
						...
						.FindByStartWithNamesAsync(nameof(Resources.DataDictionary.Banner));

				if (listTags.Any() == true)
				{
					context.HttpContext.Items[nameof(Resources.DataDictionary.Banner)] = listTags;
				}
				else
				{
					var errorMessage =
						string.Format(Resources.Messages.NotFoundError, Resources.DataDictionary.Banner);
