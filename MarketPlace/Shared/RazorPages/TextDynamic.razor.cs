...
public partial class TextDynamic
    : ComponentBaseMarketplaceFullCustom<TextDynamicResponseViewModel, TextDynamicRequestViewModel>
{
    ...
    private async Task FindByIdAndMoveToUpdateModeAsync(string id)
    {
        ...
        {
            OnUpdateMode(model: result.Value.ToTextDynamicResponse());
        }
        else
        {
            RequestResultService.AddResult(result);
            await GoToPageAsync(MarketplaceRoutes.TextDynamic(), id: null);
        }
    }
    ...
    private async Task SetListAsync()
    {
        ...
        var result = await PageSettingService.GetTextDynamicsAsync(_parameters!);
    ...
    private async Task CreateAsync()
    {
        ...
        {
            var createResult =
                await PageSettingService.CreateTextDynamicAsync(Model);
    ...
    private async Task UpdateAsync()
    {
        ...
        if (string.IsNullOrEmpty(Model.Id) == false)
        {
            ...
            {
                var updateResult =
                    await PageSettingService.UpdateTextDynamicAsync(Model);
    ...
    private async Task DeleteImageAsync(string id)
    {
        ...
        if (result is { IsSuccess: true, Value: not null })
        {
            UpdateRecordInTable(model: result.Value.ToTextDynamicResponse());
        }
    ...
    public void UpdateModelTags(Dictionary<string, string> tags)
    {
        Model.TagNames = tags.Select(x => x.Key).ToList();
    }
}