...
public partial class Banner
    : ComponentBaseMarketplaceFullCustom<BannerResponseViewModel, BannerRequestViewModel>
{
    ...
    private async Task FindByIdAndMoveToUpdateModeAsync(string id)
    {
        ...
        {
            OnUpdateMode(model: result.Value.ToBannerResponse());
        }
        else
        {
            RequestResultService.AddResult(result);
            await GoToPageAsync(MarketplaceRoutes.Banner(), id: null);
        }
    }
    ...
    private void OnFileHandleMobile(List<IFormFile>? list)
    {
        ...
        if (list is not null && list.Any() == true && list[0].Length != 0)
        {
            Model.FileMobile = list[0];
        }
        else
        {
            Model.FileMobile = null;
            result.WithError(Resources.Messages.ErroZiroByteFile);
        }
    ...
    private async Task SetListAsync()
    {
        ...
        var result = await PageSettingService.GetBannersAsync(_parameters!);
    ...
    private async Task CreateAsync()
    {
        ...
        {
            var createResult =
                await PageSettingService.CreateBannerAsync(Model);
    ...
    private async Task UpdateAsync()
    {
        ...
        if (string.IsNullOrEmpty(Model.Id) == false)
        {
            ...
            if (result.IsSuccess == true)
            {
                var updateResult = await PageSettingService.UpdateBannerAsync(Model);
    ...
    private async Task DeleteImageAsync(string id)
    {
        ...
        if (result is { IsSuccess: true, Value: not null })
        {
            UpdateRecordInTable(model: result.Value.ToBannerResponse());
        }
