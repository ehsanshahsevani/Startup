...
public partial class RoleGold
    : ComponentBaseMarketplaceFullCustom<RoleGoldResponseViewModel, RoleGoldRequestViewModel>
{
	[Inject] RoleGoldService Service { get; set; } = default!;                          
    
    private RoleGoldParameters _parameters;

    public RoleGold() : base()
    {
        _parameters = new RoleGoldParameters();

        #region PageDescriptionModel Initialize
        _pageDescription = new PageDescriptionModel(
            PageTitle: " مدیریت قوانین خرید و فروش طلا",
            null,
            [
    ...
    private async Task FindByIdAndMoveToUpdateModeAsync(string id)
    {
        ...
        else
        {
            ...
            await GoToPageAsync(MarketplaceRoutes.RoleGold(), null);
        }
    }
