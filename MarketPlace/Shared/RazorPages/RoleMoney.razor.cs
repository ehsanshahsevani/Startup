...
public partial class RoleMoney
    : ComponentBaseMarketplaceFullCustom<RoleMoneyResponseViewModel, RoleMoneyRequestViewModel>
{
	[Inject] RoleMoneyService Service { get; set; } = default!;                          
    
    private RoleMoneyParameters _parameters;

    public RoleMoney() : base()
    {
        _parameters = new RoleMoneyParameters();

        #region PageDescriptionModel Initialize
        _pageDescription = new PageDescriptionModel(
            PageTitle: " مدیریت قوانین برداشت و واریز پول",
            null,
            [
    ...
    private async Task FindByIdAndMoveToUpdateModeAsync(string id)
    {
        ...
        else
        {
            ...
            await GoToPageAsync(MarketplaceRoutes.RoleMoney(), null);
        }
    }
