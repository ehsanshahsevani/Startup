...
public partial class Faq
    : ComponentBaseMarketplaceFullCustom<FaqResponseViewModel, FaqRequestViewModel>
{
    ...
    private async Task FindByIdAndMoveToUpdateModeAsync(string id)
    {
        ...
        {
            OnUpdateMode(model: result.Value.ToFaqResponse());
        }
        else
        {
    ...
    private async Task SetListAsync()
    {
        ...
        var result = await PageSettingService.GetFaqsAsync(_parameters!);
    ...
    private async Task CreateAsync()
    {
        ...
        {
            var createResult =
                await PageSettingService.CreateFaqAsync(Model);
    ...
    private async Task UpdateAsync()
    {
        ...
        if (string.IsNullOrEmpty(Model.Id) == false)
        {
            ...
            if (result.IsSuccess == true)
            {
                var updateResult = await PageSettingService.UpdateFaqAsync(Model);
    ...
    private async Task DeleteImageAsync(string id)
    {
        ...
        if (result is { IsSuccess: true, Value: not null })
        {
            UpdateRecordInTable(model: result.Value.ToFaqResponse());
        }
