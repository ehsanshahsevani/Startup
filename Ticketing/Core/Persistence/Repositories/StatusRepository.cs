...
public class StatusRepository : Repository<Status>, IStatusRepository
{
    ...
    public async Task<Status?> FindByNameAsync(string name, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<Status?> FindUnderReviewAsync(CancellationToken cancellationToken = default)
    {
        var result = await DbSet
            .Where(x => x.IsDeleted == false)
            .Where(x => x.IsActive == true)
            .Where(x => x.Ordering == 1)
            .FirstOrDefaultAsync(cancellationToken);

        return result;
    }
    
    public override async Task<Status?> FindAsync(object id, CancellationToken cancellationToken = default)
    {
        var result = await DbSet
    ...
    public override async Task<IEnumerable<Status?>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        var result = await DbSet
            .Where(current => current.IsDeleted == false)
    ...
    /// <returns></returns>
    public async Task<PagedList<Status>> GetAllInPageAsync(
        StatusParameters parameters, CancellationToken cancellationToken = default)
    {
        ...
        var result = await PagedList
            <Status>.ToPagedList(source, parameters, cancellationToken);
