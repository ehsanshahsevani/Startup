...
///     زیر سیستم ها
/// </summary>
public class SubSystemController : BaseControllerApi
{
    public SubSystemController(IMapper mapper, HttpClient httpClient, IConfiguration configuration,
        IHttpContextAccessor httpContextAccessor, IUnitOfWork unitOfWork, ILogDetailManager logDetailManager,
        ILogServerManager logServerManager) : base(mapper, httpClient, configuration, httpContextAccessor, unitOfWork,
        logDetailManager, logServerManager)
    {
    }
    ...
    /// <returns></returns>
    [HttpGet("description/{subSystemName}")]
    ...
    public async Task<IActionResult> UpdateDescriptionAsync(string id, [FromBody] string description)
    {
        ...
        if (entity is not null)
        {
            ...
            var successMessage =
                string.Format(Messages.UpdateMessageSuccess, DataDictionary.SubSystem);
        ...
        {
            var errorMessage =
                string.Format(Messages.NotFoundError, DataDictionary.SubSystem);
