...
public class TicketViewModelFilterAction :
    IBaseAsyncActionFilter
{
    public TicketViewModelFilterAction(IUnitOfWork unitOfWork)
    {
        ...
        ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        if (tryGetValue && modelObject is TicketRequestViewModel model)
        {
            ...
            if (result.IsSuccess)
                ...
                {
                    var entity =
                        await UnitOfWork.TicketRepository.FindAsync(model.Id);

                    if (entity is null)
                    {
                        var errorMessage =
                            string.Format(Messages.NotFoundError, DataDictionary.Ticket);
