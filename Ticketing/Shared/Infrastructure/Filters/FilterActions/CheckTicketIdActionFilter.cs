...
public class CheckTicketIdActionFilter: InfrastructureSeedworks.ActionFilter.IBaseAsyncActionFilter
{
    public IUnitOfWork UnitOfWork { get; }

    public CheckTicketIdActionFilter(IUnitOfWork unitOfWork)
    {
    ...
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        else
        {
            var entity =
                await UnitOfWork
                    .TicketRepository.FindAsync(id);

            if (entity is null)
            {
                var errorMessage = string.Format(
                    Resources.Messages.NotFoundError, Resources.DataDictionary.Ticket);
